...
// -----------------------------------------------------------------------------
//
// An `absl::graveyard_flat_hash_map<K, V>` is an unordered associative container of
// unique keys and associated values designed to be a more efficient replacement
// for `std::unordered_map`. Like `unordered_map`, search, insertion, and
// deletion of map elements can be done as an `O(1)` operation. However,
// `graveyard_flat_hash_map` (and other unordered associative containers known as the
...
// `graveyard_flat_hash_map`.

#ifndef ABSL_CONTAINER_GRAVEYARD_FLAT_HASH_MAP_H_
#define ABSL_CONTAINER_GRAVEYARD_FLAT_HASH_MAP_H_
...
// -----------------------------------------------------------------------------
// absl::graveyard_flat_hash_map
// -----------------------------------------------------------------------------
//
// An `absl::graveyard_flat_hash_map<K, V>` is an unordered associative container which
...
//
// By default, `graveyard_flat_hash_map` uses the `absl::Hash` hashing framework.
// All fundamental and Abseil types that support the `absl::Hash` framework have
// a compatible equality operator for comparing insertions into `graveyard_flat_hash_map`.
// If your type is not yet supported by the `absl::Hash` framework, see
// absl/hash/hash.h for information on extending Abseil hashing to user-defined
// types.
//
// Using `absl::graveyard_flat_hash_map` at interface boundaries in dynamically loaded
// libraries (e.g. .dll, .so) is unsupported due to way `absl::Hash` values may
// be randomized across dynamically loaded libraries.
//
// NOTE: A `graveyard_flat_hash_map` stores its value types directly inside its
// implementation array to avoid memory indirection. Because a `graveyard_flat_hash_map`
// is designed to move data when rehashed, map values will not retain pointer
// stability. If you require pointer stability, or if your values are large,
// consider using `absl::graveyard_flat_hash_map<Key, std::unique_ptr<Value>>` instead.
...
//   absl::graveyard_flat_hash_map<std::string, std::string> ducks =
...
class graveyard_flat_hash_map : public absl::graveyard_container_internal::graveyard_raw_hash_map<
                          absl::container_internal::FlatHashMapPolicy<K, V>,
                          Hash, Eq, Allocator> {
  using Base = typename graveyard_flat_hash_map::graveyard_raw_hash_map;
...
template <typename K, typename V, typename H, typename E, typename A,
          typename Predicate>
typename graveyard_flat_hash_map<K, V, H, E, A>::size_type erase_if(
    graveyard_flat_hash_map<K, V, H, E, A>& c, Predicate pred) {
...
struct IsUnorderedContainer<
    absl::graveyard_flat_hash_map<Key, T, Hash, KeyEqual, Allocator>> : std::true_type {};
...
#endif  // ABSL_CONTAINER_GRAVEYARD_FLAT_HASH_MAP_H_
