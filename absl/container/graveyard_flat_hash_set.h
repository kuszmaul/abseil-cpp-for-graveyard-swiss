...
class graveyard_flat_hash_set
    : public absl::graveyard_container_internal::graveyard_raw_hash_set<
          absl::graveyard_container_internal::FlatHashSetPolicy<T>, Hash, Eq, Allocator> {
  using Base = typename graveyard_flat_hash_set::graveyard_raw_hash_set;
...
typename graveyard_flat_hash_set<T, H, E, A>::size_type erase_if(
    graveyard_flat_hash_set<T, H, E, A>& c, Predicate pred) {
...
struct IsUnorderedContainer<absl::graveyard_flat_hash_set<Key, Hash, KeyEqual, Allocator>>
    : std::true_type {};
