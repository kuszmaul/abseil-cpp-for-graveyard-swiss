...
#include "absl/container/graveyard_node_hash_set.h"
...
using SetTypes = ::testing::Types<
    graveyard_node_hash_set<int, StatefulTestingHash, StatefulTestingEqual, Alloc<int>>,
    graveyard_node_hash_set<std::string, StatefulTestingHash, StatefulTestingEqual,
                  Alloc<std::string>>,
    graveyard_node_hash_set<Enum, StatefulTestingHash, StatefulTestingEqual, Alloc<Enum>>,
    graveyard_node_hash_set<EnumClass, StatefulTestingHash, StatefulTestingEqual,
                  Alloc<EnumClass>>>;
...
TEST(NodeHashSet, MoveableNotCopyableCompiles) {
  graveyard_node_hash_set<std::unique_ptr<void*>> t;
  graveyard_node_hash_set<std::unique_ptr<void*>> u;
  u = std::move(t);
}
...
TEST(NodeHashSet, MergeExtractInsert) {
  ...
  absl::graveyard_node_hash_set<std::unique_ptr<int>, Hash, Eq> set1, set2;
...
TEST(NodeHashSet, EraseIf) {
  // Erase all elements.
  {
    graveyard_node_hash_set<int> s = {1, 2, 3, 4, 5};
