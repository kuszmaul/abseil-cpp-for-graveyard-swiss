...
#include "absl/container/graveyard_flat_hash_set.h"
...
struct BeforeMain {
  BeforeMain() {
    absl::graveyard_flat_hash_set<int> x;
    x.insert(1);
...
template <class T>
using Set =
    absl::graveyard_flat_hash_set<T, container_internal::StatefulTestingHash, container_internal::StatefulTestingEqual, container_internal::Alloc<T>>;
...
TEST(FlatHashSet, EmplaceString) {
  ...
  absl::graveyard_flat_hash_set<absl::string_view> hs(v.begin(), v.end());
...
TEST(FlatHashSet, BitfieldArgument) {
  ...
  n = 0;
  absl::graveyard_flat_hash_set<int> s = {n};
  s.insert(n);
  s.insert(s.end(), n);
  s.insert({n});
  s.erase(n);
...
TEST(FlatHashSet, MergeExtractInsert) {
  ...
  absl::graveyard_flat_hash_set<std::unique_ptr<int>, Hash, Eq> set1, set2;
...
TEST(FlatHashSet, EraseIf) {
  // Erase all elements.
  {
    graveyard_flat_hash_set<int> s = {1, 2, 3, 4, 5};
