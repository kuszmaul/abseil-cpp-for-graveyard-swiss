...
template <class T>
using Set =
    graveyard::flat_hash_set<T, StatefulTestingHash, StatefulTestingEqual, Alloc<T>>;
...
TEST(FlatHashSet, EmplaceString) {
  ...
  graveyard::flat_hash_set<absl::string_view> hs(v.begin(), v.end());
...
TEST(FlatHashSet, BitfieldArgument) {
  ...
  n = 0;
  graveyard::flat_hash_set<int> s = {n};
  s.insert(n);
  s.insert(s.end(), n);
  s.insert({n});
  s.erase(n);
...
TEST(FlatHashSet, MergeExtractInsert) {
  ...
  graveyard::flat_hash_set<std::unique_ptr<int>, Hash, Eq> set1, set2;
...
TEST(FlatHashSet, EraseIf) {
  // Erase all elements.
  {
    graveyard::flat_hash_set<int> s = {1, 2, 3, 4, 5};
