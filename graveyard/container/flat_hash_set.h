...
// -----------------------------------------------------------------------------
//
// A `graveyard::flat_hash_set<T>` is an unordered associative container
// designed to be a more efficient replacement for `std::unordered_set`. Like
...
// `flat_hash_set`.
//
// The `graveyard::flat_hash_set` implementation is based on
// `absl::flat_hash_set`.  The graveyard version uses linear probing and
// graveyard hashing, whereas the absl version uses quadratic probing.  The absl
// version requires the table size to be a power of two.  The graveyard version
// can have arbitrary sizes, which probably saves 30% of memory on average.  The
// graveyard version also can run at much higher load factors.
#ifndef GRAVEYARD_CONTAINER_FLAT_HASH_SET_H_
#define GRAVEYARD_CONTAINER_FLAT_HASH_SET_H_
...
// -----------------------------------------------------------------------------
// graveyard::flat_hash_set
// -----------------------------------------------------------------------------
//
// A `graveyard::flat_hash_set<T>` is an unordered associative container which
...
// types.
//
// Using `graveyard::flat_hash_set` at interface boundaries in dynamically
// loaded libraries (e.g. .dll, .so) is unsupported due to way `absl::Hash`
// values may be randomized across dynamically loaded libraries.
//
...
//  graveyard::flat_hash_set<std::string> ducks =
//     {"huey", "dewey", "louie"};
//
...
struct IsUnorderedContainer<graveyard::flat_hash_set<Key, Hash, KeyEqual, Allocator>>
    : std::true_type {};

}  // namespace container_algorithm_internal
}  // namespace absl

#endif  // GRAVEYARD_CONTAINER_FLAT_HASH_SET_H_
